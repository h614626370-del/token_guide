...
::: tip 统一接口地址
OpenAI 兼容客户端、Codex CLI、Claude Code、OpenCode、OpenClaw 当前都优先使用 `https://kkflow.org/v1`。
:::
...
模型建议：优先使用 `gpt-5.6-sol`。其他可选模型以“使用密钥”页面展示为准。
...
`model_provider` 必须和下方 `[model_providers.kkflow]` 的名称一致；不要只改其中一处。使用这类非 OpenAI、非 Azure 的自定义 Provider 时，Codex 会在本地完成上下文整理和自动压缩。
...
::: tip 完成配置后的使用建议
配置文件写好后，重新启动 VS Code 或你正在使用的编辑器。模型优先使用 `gpt-5.6-sol`；如需切换其他模型，请先在“使用密钥”页面确认当前密钥分组是否开放。权限模式建议选择“完全访问权限”。
:::
...
`baseUrl` 建议填写 `https://kkflow.org/v1`，不要末尾再加 `/`。`primary` 必须带 provider 前缀，例如 `openai/gpt-5.5`。若报 `Gateway start blocked`，确认 `gateway.mode` 已设置为 `local`。
:::
...
如果你的客户端仍基于 Chat Completions，可改用 `https://kkflow.org/v1/chat/completions`，核心参数对应为 `messages`、`temperature`、`top_p`、`max_tokens`。

## 10. gpt-image-2 调用参数（Images）

文生图接口：`https://kkflow.org/v1/images/generations`。常用参数如下：
...
如果是基于参考图编辑，请使用 `https://kkflow.org/v1/images/edits` 并改为 `multipart/form-data` 上传 `image`，可选 `mask`。
...
`404`：接口路径错误。OpenAI 兼容客户端请优先使用 `https://kkflow.org/v1`。
