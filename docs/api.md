...
模型建议：优先使用 `gpt-5.5`。其他可选模型以“使用密钥”页面展示为准。
...
::: tip 完成配置后的使用建议
配置文件写好后，重新启动 VS Code 或你正在使用的编辑器。模型优先使用 `gpt-5.5`；如需切换其他模型，请先在“使用密钥”页面确认当前密钥分组是否开放。权限模式建议选择“完全访问权限”。
:::
...
| 类型 | 模型 | 适用场景 |
| --- | --- | --- |
| 优先模型 | `gpt-5.5` | 综合能力强，适合作为默认与编程项目开发 |
