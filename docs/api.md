...
`baseUrl` 建议填写 [https://kkflow.org/v1](https://kkflow.org/v1)，不要末尾再加 `/`。`primary` 必须带 provider 前缀，例如 `openai/gpt-5.5`。若报 `Gateway start blocked`，确认 `gateway.mode` 已设置为 `local`。
:::
