        ...
        <div class="controls-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 1rem;">
            ${Object.values(ACTIONS).map(action => `
                <div class="control-item" style="background: rgba(255,255,255,0.05); padding: 1rem; border-radius: 4px; display: flex; justify-content: space-between; align-items: center;">
                    <span style="color: var(--text-dim); font-size: 0.8rem;">${actionLabel(action)}</span>
        ...
        <div class="routine-steps" style="margin-bottom: 2rem;">
            ${routine.steps.map((s, i) => `
                ...
                ">
                    ...
                    <span style="color: var(--text-dim); font-size: 0.85rem;">${difficultyLabel(s.difficulty)}</span>
...
function difficultyLabel(value) {
    return { easy: '简单', medium: '普通', hard: '困难' }[value] || value;
}

function actionLabel(value) {
    return {
        fire: '开火', ads: '瞄准', reload: '换弹', restart: '重新开始', pause: '暂停',
        weapon1: '武器 1', weapon2: '武器 2', weapon3: '武器 3', weapon4: '武器 4',
        sens_up: '提高灵敏度', sens_down: '降低灵敏度',
    }[value] || value;
}
...
async function runRoutineStep() {
    ...
    if (!step) {
        showNotification('🎉 训练日程完成！', 'success');
        router.showView('menu');
        return;
    }
    ...
    if (!modeHandler) {
        showNotification(`错误：未知训练模式 ${step.mode}`, 'error');
        return;
    }
    ...
    // Smooth transition
    titleEl.innerHTML = `<span style="color: var(--primary)">第 ${step.stepIndex} / ${step.totalSteps} 项</span>`;
    ...
    if (state.engine) {
        state.engine.init(step.mode, step.difficulty, modeHandler, {
            ...
            onComplete: (result) => {
                ...
                // For now, auto-next
                showNotification(`本项完成！得分：${result.score}`, 'success');
