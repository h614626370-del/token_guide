...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help) echo "用法: bash setup-macos.sh [--no-launch]"; exit 0 ;;
    *) fail "未知参数: $1" ;;
  esac
done
...
install_node() {
  ...
  archive_name="node-${NODE_VERSION}-darwin-${arch}.tar.gz"
  temp_dir="$(mktemp -d)"
  archive="$temp_dir/$archive_name"
  dest="$HOME/.local/lib/nodejs/node-${NODE_VERSION}-darwin-${arch}"
