...
if ($Help) {
    Write-Host "用法: .\setup.ps1 [-Model MODEL] [-NoLaunch]"
    Write-Host "默认模型: gpt-5.6-sol"
    Write-Host "Codex CLI 安装完成后会询问是否更新 API Key。"
    exit 0
}
...
if ($codexCommand) {
    ...
    try { Write-Host "  版本: $(& $codexPath --version)" -ForegroundColor DarkGray } catch {}
} else {
...
if (-not $ApiKey) {
    ...
    if (-not $choice) { $choice = Read-Host "Codex CLI 安装完成，是否更新 API Key？[y/N]" }
    if ($choice -notmatch '^(?i:y|yes)$') {
        $skipConfig = $true
        Write-Host "  已选择不更新 API Key，跳过中转站配置" -ForegroundColor Yellow
    } else {
...
$authorizationHeader = ConvertTo-TomlString ("Bearer " + $ApiKey)
$providerBlock = @(
...
Write-Host "  API Key 已写入 Provider 配置" -ForegroundColor Green
