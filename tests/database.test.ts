...
describe('database migrations', () => {
  ...
  it('can reopen an existing database without reapplying migrations or seeds', () => {
    ...
    try {
      firstAppliedAt = first.prepare('SELECT applied_at FROM schema_migrations WHERE id = 8').get()
    } finally {
      first.close()
    }
    ...
    try {
      ...
      expect(second.prepare('SELECT applied_at FROM schema_migrations WHERE id = 8').get()).toEqual(firstAppliedAt)
    } finally {
      second.close()
    }
  })
